...
export class IspComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.ispService
                ...
                .subscribe(
                    (res: HttpResponse<IIsp[]>) => this.paginateIsps(res.body, res.headers),
    ...
    registerChangeInIsps() {
        this.eventSubscriber = this.eventManager.subscribe('ispListModification', response => this.reset());
    }
    ...
    protected paginateIsps(data: IIsp[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.isps.push(data[i]);
        }
    }
